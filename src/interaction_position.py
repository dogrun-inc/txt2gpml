...
def main(source: dict, nodes:dict):
    ...
    for i in filter(lambda inter: inter.get('has_anchor') == False, interactions):
        ...
        iteraction_position[i["GraphId"]] = grxy.main(nodes[i["start_point"]], nodes[i["end_point"]])
        iteraction_position[i["GraphId"]]['start_point']['GraphRef'] = i['start_point']
        iteraction_position[i["GraphId"]]['end_point']['GraphRef'] = i['end_point']

    # pathway_attributesの最終出力にanchorを持つinteractionの情報の追加
    for i in filter(lambda inter: inter.get('has_anchor') == True, interactions):
        anchor = next(a for a in source['anchors'] if a['GraphId'] == i['end_point'])
        # anchorの座標を取得する。anchorの置かれたinteractionの両端の座標　と相対位置を引数にする
        # anchorはinteractionの"end_point"(終点側)に置くように運用的に固定する
        ...
        # anchor情報を追加
        iteraction_position[i["GraphId"]] = grxy.main(nodes[i["start_point"]], end_point, end_rel=False)
        # interactionの両端の情報を追加        
        iteraction_position[i["GraphId"]]['start_point']['GraphRef'] = i['start_point']
        iteraction_position[i["GraphId"]]['end_point']['GraphRef'] = i['end_point']
